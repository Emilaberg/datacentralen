 ...
 */
  const AuthMe = async (username:string, password:string, enableLog = LogUserNameAndPasswordInAuthMe) => {
    ...
    try {
      ...
      if(enableLog)console.log(error, "ERROR: Unable to authenticate credentials");
      return null;
    }

    if (!responseFromApi || !responseFromApi.ok) {
      if(enableLog)console.error("Failed to authenticate:", responseFromApi?.status, responseFromApi?.statusText);
      return null;
    }
 ...
 */
  const SendAuthorizedRequest = async (requestProps: RequestProps) => {
    ...
    if(!authToken) {
      if(requestProps.enableLog)console.error("ERROR: Could not find auth token, and could not make a new one. Is the API running?");
      return null;
    }
    ...
    try {
      ...
      if(!responseFromApi) {
        if(requestProps.enableLog)console.error("ERROR: Could not send request to API. Check console for details.");        
        return null;
      }
      return responseFromApi;
      ...
      if(requestProps.enableLog)console.error("ERROR: Could not send request to API. Check console for details.", error);
    }
 ...
 * 
 */
  const PUTArticleChangeContent = async (id: number, data: string, enableLog = defaultLogRequest) => {
    try {
      const response = await SendAuthorizedRequest({
   ...
   */
  const DELETEArticleContent = async (id: number, enableLog = defaultLogRequest) => {
    const response = await SendAuthorizedRequest(
      {
    ...
    if (!response || !response.ok) {
      if (enableLog) console.error("Failed to remove content from article:", response?.status, response?.statusText);
      return null;
    }
    return response;
  }
  ...
  const DELETEArticle = async (id: number, enableLog = defaultLogRequest) => {
    ...
    if (!response || !response.ok) {
      if (enableLog)console.error("Failed to delete article:", response?.status, response?.statusText);
      return null;
    }
    return response;
  }
  ...
  const GETAllArticles = async(enableLog = defaultLogRequest)=>{
    ...
    if (!response || !response.ok) {
      if (enableLog)console.error("Failed to fetch all articles:", response?.status, response?.statusText);
      return null;
    }
    return response
  }
   ...
   */
  const GETArticleById = async(idOfArticle : number, enableLog = defaultLogRequest) =>{
    const response = await SendAuthorizedRequest(
      {
    ...
    if (!response || !response.ok) {
      if (enableLog)console.error(`Failed to fetch article by ID (${idOfArticle}):`, response?.status, response?.statusText);
      return null;
    }
    return response
  }
   ...
   */
  const GETAllArticlesWithTitleAndDescription = async(enableLog = defaultLogRequest) =>{
    const response = await SendAuthorizedRequest(
      {
    ...
    if (!response || !response.ok) {
      if (enableLog)console.error("Failed to fetch all articles with titles and descriptions:", response?.status, response?.statusText);
      return null;
    }
    return response
  }
  ...
  const GETAllArticlesAsCardDTO = async (enableLog = defaultLogRequest) => {
    ...
    if (!response || !response.ok) {
      if (enableLog)console.error("Failed to fetch all articles as CardDTO:", response?.status, response?.statusText);
      return null;
    }
    return response
  }
